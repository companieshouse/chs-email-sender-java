...
class NotificationApiClientTest {
    ...
    private static final String POST_FAILED_MSG = "POST failed for appId: %s with  status code: [%d]";
    ...
    void postEmailShouldDelegateToResponseHandlerWhenApiReturnsBadRequest() {
        ...
        verify(responseHandler, times(1))
                .handle(ex, POST_FAILED_MSG.formatted(APP_ID, HttpStatus.BAD_REQUEST.value()));
    }
    ...
    void postEmailShouldDelegateToResponseHandlerWhenApiReturnsServerError() {
        ...
        verify(responseHandler, times(1))
                .handle(ex, POST_FAILED_MSG.formatted(APP_ID, HttpStatus.INTERNAL_SERVER_ERROR.value()));
    }
    ...
    @Test
    void postEmailShouldDelegateToResponseHandlerWhenUrlIsInvalid() {
        ResourceAccessException invalidUrlError = new ResourceAccessException(
                "I/O error on POST request for \"http://invalid-host/email\": Connection refused");
        stubChain(invalidUrlError);

        notificationApiClient.postEmail(TEMPLATE_NAME, APP_ID, DATA);

        verify(responseHandler, times(1))
                .handle(invalidUrlError, "POST %s failed due to connection error".formatted(EMAIL_URI));
    }
}
